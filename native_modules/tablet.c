...
void parse_tablet_avg(int buffer_fd, int tablet_fd, struct tablet_config tablet, struct display_config display) {
  int x = 0;
  int y = 0;
  double x_scaled = 0;
  double y_scaled = 0;
...
void init_tablet(const char *name, const char *hidraw_path, struct tablet_config tablet, struct display_config display) {
  ...
  case STANDARD_PARSER:
    printf("Using standard parser\n");
    parse_tablet_buffer(buffer_fd, tablet_input_fd, tablet, display);
    break;
  case DOUBLE_REPORT_PARSER:
    printf("Using double report parser\n");
    parse_tablet_buffer_interpolated(buffer_fd, tablet_input_fd, tablet, display);
    break;
  case AVERAGE_BUFFERED_REPORT_PARSER:
    printf("Using buffered avg parser\n");
    parse_tablet_buffer_avg(buffer_fd, tablet_input_fd, tablet, display);
    break;
